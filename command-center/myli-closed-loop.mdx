...
> **the RELATIONSHIP** — the G0→G7 ladder, cadence, compliance and temperature.
...
| The ledger wire the frames advance on | `writeLedgerSlot` | **FED — corrected 8/27.** This row read "STARVED — zero callers… or gates-to-gates is dead on arrival" from 8/23 until today. The S2 lane landed the feeders and no doc was updated. Measured: `lib/ai/convo-state.ts:163` defines it; **:198, :207, :223, :224, :246** call it, under a header at :170 that says so in the code's own past tense — *"the starvation the S1 receipt flagged: writeLedgerSlot **had** ZERO…"*. **Gates-to-gates is not dead on arrival.** |
...
| Quiet hours | `QUIET_HOURS` reason code | **REAL — corrected 8/27.** This row read "FAKE GATE — zero producers" from 8/23 until today. Measured: `lib/playbooks/engine.ts:1793, 1803, 1821` writes a denied trace row naming `QUIET_HOURS` **plus a pending scheduled action with a release time** — a visible, scheduled deferral, which is exactly the "delays LOUDLY, never a hidden clamp" the containment law demanded. `lib/business-hours.ts:101-121` is the reader (its `catch` returns the default with the comment *"a DB blip must never be read as '3 AM is fine'"* — fail-closed, correctly), and `components/myli-desk/desk-state.ts:350` renders the reason to a human as *"it was outside sending hours."* |

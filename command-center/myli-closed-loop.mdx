...
- **C1 · "AI-Trav" — reference not found in the record.** No document, table, or code token
  matching `ai_trav` exists anywhere in the tree. The nearest record artifact is the SureSend
  teardown. If AI-Trav is a specific SureSend persona Zach has seen, it needs one capture pass
  before anyone builds "to" it from memory.

...
| **Goal frames** (the closest thing to playbook CONTENT) | `lib/ai/goal-frames.ts` | 4 frames: `buyer_book_showing` · `buyer_lender` · `buyer_nurture` · `seller_listing_appt`. Ordered required-slot ladders + missions + exits |
...
**The slot ladder is NOT invented here** — it is `lib/ai/goal-frames.ts`'s `buyer_book_showing`
...
| **Quiet hours** | ⚠ **GAP — see §7 OQ-3.** `QUIET_HOURS` is a declared reason code with **zero producers** repo-wide. A declared gate point with no enforcement is the fake-gate shape. It must become a **VISIBLE send-policy node** (containment law), never a hidden clamp — and it must be real before ① speaks |

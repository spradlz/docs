...
| **G1 — detection on a clock** | in-app detection runs scheduled, cursor-correct, heartbeat it writes itself | live daemon + first-run receipt; then the Inngest port with its first scheduled `hl_run_log` row | ✅ **SIGNED + PORT COMPLETE 8/23 03:43Z** — detection now owned by the registered Inngest fn (elected via inapp_detector_source, WALKING every minute, witnessed in hl_run_log); the pm2 twin stopped + quarantined 03:33; the probe reads the run log (the beat written where it's read). 9s detection stands as the measured record |
...
| **G4 — every notification road proven** | each of the 9 mint triggers + notify kinds fires a real bell/text through the ONE D4 point; every toggle actually gates | per-kind live-fire receipts; a toggle flipped off provably stops the send | ✅ **SIGNED 8/23** — 9/9 mint roads proven twice-consecutively (card+bell, wire-refusals recorded); the toggle census complete (2 fake flags filed, ai_master polarity filed as Zach's open ruling); RED-3 cured (5s TTL, guard-armed probe); RED-2 closed REFUSED-BY-DESIGN (the 6/23 email ban + call_note, loud run-log refusals); the wire-contract P0 fixed+deployed+ratcheted; the unfalsifiable-proof class cured (per-arm bodies). Receipts: captain-g4 + builder-g4b |

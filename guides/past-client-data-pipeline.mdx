...
The human-readable proof surface for this pipeline lives on its own page: **[Audit Report](/guides/audit-report)** (`Seller Report → Audit Report`). It reads `resolver_block` live and shows every client's transaction history, the resolution matrix (with `*_source` provenance), and the Follow-Up Boss dossier note, each linked to the client's FUB profile.

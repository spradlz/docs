...
description: "State engine · playbooks · the Director · Myli's Desk (the review queue) · the prompts — five sections of ONE document, per Zach's rulings #37 and #50. Carries his signature block and the three signed addenda verbatim."
---
...
⚠ **ONE AMENDMENT TO THE RULING ABOVE, MADE BY ZACH THE SAME DAY.** Ruling #37 names six things;
**ruling #50 moved the sixth out** — *"THE AI TEST COCKPIT LIVES OUTSIDE THE MYLI OS (changed from
the earlier section map — it is its own canonical doc/initiative)."* His #37 quote is left exactly as
he said it, because a signature is not edited to match a later one; this note is how the two are
reconciled. **So the Myli OS is FIVE sections**, and the cockpit is
**[its own document](/guides/ai-test-cockpit)**.

**It replaces five pages, which were retired into it on 2026-08-27:** `the-myli-engine`,
`myli-state-master`, `myli-closed-loop`, `myli-playbooks-spec` and `ai-programming`. Their URLs
redirect here; the legacy `test-harness` redirects to the cockpit doc instead. Nothing was
summarized away — where a page carried a signature, a wording bank or a measured table, it moved
verbatim.
...
⚠ **This page is long on purpose and that was ruled, not chosen.** Folding five pages into one
...
| **§5** | **The prompts** | The prompt registry, the assembly order, the voice law, and the per-rung wording |
| — | **Shipped and undocumented** | Live modules with tests and zero mention in any doc |

**⭐ THE AI TEST COCKPIT IS NOT IN HERE, BY RULING.** Zach moved it out on 2026-08-27 (ruling #50):
*"THE AI TEST COCKPIT LIVES OUTSIDE THE MYLI OS — it is its own canonical doc/initiative."* The
boundary is his ruling #34: the review queue (§4) is the **live** surface where he reviews real
work; the cockpit is the **editing and testing** place. Two surfaces, two jobs, two documents.
**→ [THE AI TEST COCKPIT](/guides/ai-test-cockpit).**

**Also NOT in here, deliberately:** the [AI Containment Law](/command-center/ai-containment-law)
...
linked from the cockpit doc). The two generated boards, [the state-engine
...
### The web chat's turn contract (shipped 2026-09-09, lane-webchat)

The website chat (`components/site/AiSearchModal.tsx` + `app/api/search/nlp/route.ts` + `lib/search/agent-voice.ts`) now runs on three mechanical rules, built to the desk's ChatGPT second opinion (`.claude/reports/grinder/whole-loop/SECOND-OPINION-webchat.md`) and measured in an own headless chromium (iPhone 14 emulation) on localhost before the push:

**1. One active turn, a queue, one cleanup.** Exactly one network-active turn per conversation (`activeTurnRef`, read from the ref, never a closure). A line typed while a turn is live is accepted synchronously: its bubble paints at the tail marked *Queued* (`data-hl-pending="queued"`) and runs FIFO when the turn ends. A line typed during turn zero preempts the opener (its fetch is aborted, its late answer discarded). Every user bubble carries `pending: sending → cleared | failed` ("Didn't send" + the floor's *Ask again*). Every turn ends through one routine, `endTurn(gen)` — done, error, abort (New chat / close under a live turn), or the 150 s watchdog — and late events from an ended turn are dropped by `guardedHandlers`. *Measured: before, 3 of 5 typed sends during the opener stuck the header or never posted; after, 10 of 10 across the whole opener window painted in 5–16 ms and 0 stuck.*

**2. Painted prose is immutable.** On a streamed turn no guard re-authors her words: a per-paragraph gate runs the ungrounded-number check before a paragraph is forwarded (a failing paragraph holds the tail); after anything has streamed the ungrounded / act-truth / claims-without-evidence guards strip deterministically and the length guard stands down; the ack-echo guard trims the first paragraph *before* it paints (the first reveal waits for a second paragraph so an echo-only opener can be judged with what follows). The packaging round is off on a presenting search turn — the route's `bindStateRow` builds the pill row from the state table either way — and a reply cut at `max_tokens` (1024) drops its unfinished last paragraph in code. *Measured on the rich search ask: 43.8 s with a 6.9 s silent stall and a mid-heading cut → 28.3 s, last delta +27.9 s, done +28.3 s, zero `revise` frames.*

**3. Turn zero is a shell, not a model call.** The chat opens with the static line, the entry pills and a code-owned state label ("Welcome back — you were working out what you can afford", from the newest thread within the resume window) in tens of milliseconds, with no *Thinking…* in the header. A stranger gets no model opener at all (the quiet probe answers `known:false`); a known person's opener is composed quietly and **appends** under the shell with the shell's pills spent — it never replaces painted text, and its request carries the do-not-greet-again context. The home page's seed (a tapped pill or a typed ask) is the person's first turn, not an offered sentence; the `?aiq=` deep link keeps the offered seed. *Measured: shell +3–10 ms on a fresh, a signed-in and a second-window open; the label at +159 ms.*

### The tap order, the funnel pill law and the deterministic wait (2026-09-09, lane-webchat-2 — Zach's 06:16 ET prod rulings #257.373–.375)

Zach walked production on his iPhone at 06:16 ET and ruled three things; each is now code, red-proven, and measured on a recording of his exact road (open the chat → tap **What can I afford?** → set the income slider to $8,000 by touch → **That's about right** → wait → read the pills). The before-recording on prod and the after-recording on localhost sit beside the receipt (`.claude/reports/grinder/whole-loop/RECEIPT-webchat-2-2026-09-09.md`).

**4. The tap order is fixed (#257.373).** A pill tap paints the buyer's own bubble first, then her reaction, then her status lines, and only then the card — never the card first. Measured on prod before this: the income slider painted at +0 ms with its commit button reading "One sec — her question is on its way", her reaction landed *under* it, and at the end of the turn the early card vanished and a second slider painted under her question. Now the step's control (`pendingControl`) renders in a **tail slot** below the reaction and the status line, only once her reaction has landed, with its real commit label (a tap while she is still writing is queued by the turn contract, never dropped), and it **stays** across `done`: when her question names the same fact, the message's own slider stands down (`tailHoldsControl`) and the card the buyer already saw keeps its value. A money answer pre-paints the *next* step's card (income → debts → down payment, the funnel's order in code) under her reaction the moment it lands. Her reaction and her answer are never the same sentence twice: `dropAckEcho` trims a leading echo from the answer (as a whole, a leading paragraph, or a leading sentence) before either bubble is filed.

**5. The funnel pill law lives in code, not in her prose (#257.374).** The route decides the funnel position from the transcript's committed answers (`funnelStanding`, the same predicate the ack reads) and hands it to the registry as `funnelByCode`. That field outranks every reading of *this* reply's wording: measured on prod, her debts question read "now I need to know what you're paying toward other debts each month" — no question mark, none of the debts words — so the row fell to `NOTHING_IN_HAND` and drew four capability pills under a mid-funnel question. Now the same sentence is a `FUNNEL_QUESTION` (cap 0), the slider is declared from the position when the prose readers find nothing, and the client draws exactly the control, its own **Not sure**, and one **Browse homes** exit (`myli-chat-funnel-exit`) — never a capability pill from outside the funnel. A turn the math delivered still outranks the position. Outside a funnel, the two or three next steps she names at the end of her answer are read (`nextStepsFromReply`) and bound through the same door as every pill, and they lead the row; a step no registry tool does is dropped by name (the receipt lists them as cards). Proof: `npx tsx lib/myli/contract/funnel-by-code.test.ts` (18 arms, every plant with its control, the first arm being Zach's exact sentence).

**6. The wait is a deterministic bank, not a model (#257.375 — Zach overruling the second opinion, recorded in `SECOND-OPINION-webchat.md → Departed`).** `lib/myli/status-lines.ts` holds 10–20 pre-written status lines per task class (affordability · home search · one home · neighborhood · saved search · tour · general) with merge fields (`{city} {address} {min} {max} {budget} {n} {step} {of}`) filled only from the turn's known inputs — the search she is showing, the page's home, the committed facts — so a line whose field is unknown is not shown and no count is ever invented. The class comes from what was tapped or typed and the funnel's fact, never from a model. The first line paints in the same frame as the buyer's bubble, before any network; the rest cycle at ~2.2 s (`MyliWait.tsx`), and a real `tool_start` line still wins the slot while it runs. These strings are UI chrome the code writes about its own work and are never read by a model, which is why they live in their own file and the script cage (`no-scripted-line`) stays green. Measured on prod before this: zero status lines on both affordability turns (the model-written plan had been retired on 9/08 and the funnel runs no tools) — a bare mark for 6 s and then 12 s. Proof: `npx tsx lib/myli/status-lines.test.ts`.

### The funnel body — five corrections from Zach's 14:15 ET prod walk (2026-09-09, lane-sonnet-webchat-r2 — ruling #257.382)

Zach walked prod again (sha `631471b5ef`) eight hours after the 06:16 ET rulings above landed and found five things the funnel body still got wrong, in his own order. Companion brief/receipt: `.claude/reports/grinder/whole-loop/RECEIPT-webchat-r2-2026-09-09.md`.

**Z4 (P0, measured first) — the funnel position advances only on a captured fact, never on a captured question.** He typed a clarifying question under the debts step ("I'm not sure what counts as monthly debt — do medical bills or a past-tax payment plan count?") and the position skipped straight to DOWN PAYMENT with no debts value ever committed. Root cause: `funnelStanding` (`app/api/search/nlp/route.ts`) decided "was this field answered" with `ANSWERY_LINE = /\d|\b(none|nothing|zero|excellent|good|fair|poor|building|not sure|no idea)\b/i` — a plain "does one of these words appear ANYWHERE" test, and his clarifying sentence contains the literal phrase "not sure". Fixed by routing both of `funnelStanding`'s call sites through `committedMoneyLine` (`lib/myli/contract/registry.ts`, newly exported) — the SAME anchored gate (`^...$`, the line must OPEN with the figure) that `typedCommit`/`factsCommitted` already use for the running number and the delivery gate: one predicate for "did they commit this fact", not two. Proved on a fresh localhost recording (`scratchpad/webchat2/road-z4.mjs`, session scratch): two clarifying turns after income → the field stayed DEBTS; "about $400 a month in debts" → the field advanced to DOWN PAYMENT. Unit proof: `lib/myli/contract/funnel-by-code.test.ts`, arm "#257.382 Z4" — his exact sentence as the CONTROL, plus a plant that mentions a dollar figure inside a question and still commits nothing (the anchor, not the absence of digits, is what matters).

**Z1 — the next step's card paints only after her question for that step has painted, on every step.** The tail slot (`tailSlotOn`, `AiSearchModal.tsx`) previously painted once her ACK/reaction had landed (`!!wait.ack`), which is well before her literal answer text streams in — this modal never streams the answer itself ("the answer arrives whole"), so "her question has painted" and "the turn stopped loading" are the same event on this architecture. Frame-level measurement (`scratchpad/webchat2/road-z1.mjs`) on the pre-fix tree: the debts card mounted at +3.4s while her literal "Question 2 of 4" text did not land until +10.1s. Fixed: `tailSlotOn` no longer paints during `loading` on the streaming road at all (`loading ? !streamOn : true`) — it now paints in the same render as the answer.

**Z3 — no [Browse homes] on a funnel turn.** Zach: *"I don't even think we need Browse Homes on these funnel things… On the funnels we just say That's about right, Not sure, and then the next steps."* The `myli-chat-funnel-exit` pill row (both the block-road and tail-slot renditions) is retired; the funnel row is now exactly the answer control plus its own built-in **Not sure** — the composer (a typed sentence) is the only way out of a funnel turn.

**Z5 — a clarifying turn inside a funnel gets its own honest status lines, never the funnel's math.** "Lining up question 2 of 4 · Checking today's rates" ran while he was answering "does tuition count as debt?" — telemetry describing work that was not happening. `status-lines.ts` gains an eighth class, `funnel_clarifying` (no merge fields — a generic aside, never a step number or a city), selected in `send()` when her last message declared a funnel money question (`tailHersFact`) and the current line does not capture it (`committedMoneyLine`, the same Z4 predicate) — never a second copy of "was this answered."

**Z2 — the ack's warmth is a judge row, not a code change.** "You're looking at what you can afford, and you're bringing in about $8,000 a month" read flat. Per THE_UNHOBBLING (context, never a script), this is not fixed by writing her a sentence — it is a matrix row, `ACK-ACKNOWLEDGES-THE-ANSWER-NOT-THE-GOAL`, added to `tools/flow-judge/journeys/myli-consumer.journey.json`, which fails on an ack that restates the funnel's GOAL instead of acknowledging the number and moving on. The ack's own context lives in `lib/myli/ack-call.ts`, owned by a different lane at the time of this pass — routed to the desk rather than edited out of scope.

### The webchat → CRM projector, and the ordering that stopped starving it (#257.611, shipped 2026-09-12)

Every webchat turn is appended to the ledger (`hl_chat_events`, written only by `appendChatEvent()` in
`lib/myli/chat-ledger.ts` — the one authority for *"what happened in this conversation"*) and, in the
same beat, an **outbox row** is committed to `hl_chat_outbox`. One projector drains that outbox into
the CRM's `hl_messages`. The outbox row is the durable record; the `myli/chat-outbox.enqueued` event is
only a nudge, so a lost event costs latency, never a turn.

- **The drain:** `lib/myli/chat-outbox-drain.ts` (`drainChatOutbox`), dispatched by the
  `chat-outbox-drain` Inngest function — a one-minute cron plus the nudge event.
- A drained row is **never deleted**: `projected_at` is stamped, and that stamp is the receipt.

<Warning>
**THE INCIDENT — a fixed order over a fixed set is not a queue, it is a window.** The selection was
`WHERE projected_at IS NULL ORDER BY conversation_id, sequence_no LIMIT 200`: a bound on **rows**,
ordered by a key that never changes. 424 pending rows sat on anonymous conversations whose ids happen
to sort early and correctly refuse to project (`NO_LEAD` — there is no person to file them under), and
they filled all 200 slots of every pass. **120 consecutive runs returned `pending: 200, projected: 0`
with a byte-identical list, while 275 real turns across 31 conversations — five carrying a resolvable
lead — sat at position 201 and were never once looked at. 20 hours 19 minutes, with a green job and a
green cron.**
</Warning>

**The fix: bound the pass by CONVERSATION, and reserve part of the budget.** A conversation is the unit
that costs something (one identity resolution, one ledger read, one POST), and the order over
conversations has to move:

| Lane | Slots | Order | Why |
|---|---|---|---|
| **Fresh** | 30 | conversations holding a never-attempted row, **oldest first** | oldest-first is self-correcting — waiting can only improve your position |
| **Rotation** | 10, **reserved** | everything the fresh lane did not take, **fewest attempts first** | guarantees a parked conversation is served within roughly `pending_conversations / 10` passes |

⭐ **The first version of this fix was the same defect with a new head of line, and its own control
caught it.** That version ordered conversations `has_fresh DESC, min_attempts ASC` — an *absolute*
priority for conversations holding a never-attempted row. Simulated against a queue where fresh traffic
never lets up, it served only 67 of 106 conversations, because 31 permanently-fresh conversations held
31 of the 40 slots. **An absolute priority is a starvation bug that has not met its workload yet.**
Replacing a fixed order with a priority order does not remove head-of-line blocking; it renames the
head. Only a **reserved** budget creates the bound.

<Note>
**`NO_LEAD` is a standing condition, not a permanent one.** Those rows are anonymous visitors we cannot
yet file under a person; they drain the moment that visitor registers. Dead-lettering them would look
like a tidy fix and would silently drop real conversations. The `NO_LEAD` path now bumps `attempts`
(where it deliberately did not before) precisely so the rotation lane can order by it.
</Note>
...
<Info>
**⭐ SIGNED v2 — ruling #100.1 (Zach, 2026-08-31: "SCORING V2: SIGNED AS DRAFTED").** The v1
uncapped sum below this section's earlier drafts was falsified by the book (27 of 60 real leads
parked at exactly 100 — the scoring audit, ruling #99.3) and replaced by the impulse register.
The doctrine, verbatim: *"the only 100 is a tour request or a brand-new lead; from there they are
steady-state or cooling, and our job is to bring them back up to 100."* Model + autopsy:
`.claude/reports/scoring-audit-2026-08-31/REPORT.md`. Implementation: `computeScore` in
`lib/ai/scenario-reader/dials.ts` (the ONE reader); the table: `THE_IMPULSES` in
`lib/ai/scenario-reader/constants.ts`.
</Info>
...
**🌡 TEMPERATURE — the level, as an IMPULSE REGISTER.** Answers: *how alive is this relationship
right now?* Each event fires an impulse (peak × 0.5^(age/half-life)); the score is the **MAX of
the live impulses** — impulses never stack, so no amount of noise sums past its class. Decay
glides toward a lifecycle **resting floor** (active ≤30d: 30 · nurture 30–90d: 15 · dormant: 5 —
never zero while the relationship lives); the five **negative states** push down or freeze.
Bands: HOT at 70 and above · WARM 40–69 · COOL 15–39 · COLD below 15.
...
##### The impulse table (SIGNED v2, #100.1 — only two events read 100)

| Event (catalog kind) | Peak | Half-life | Why |
|---|---|---|---|
| **Tour requested (lead-asked)** | **100** | 7d | The apex hand-raise. Refreshes, never stacks; a classified cancellation **revokes** it. The 24h HOT band floor still rides it. |
| **Brand-new lead (born / registered)** | **100** | **3d** | The other ruled 100. New leads cool FAST without engagement — the speed-to-lead clock rides this slope. |
| Inbound reply (text / call / Zillow message / email), content positive or neutral | 85 | 7d | A human speaking outranks every model — but every inbound is **classified first** (§ the Bradley rule below); a pause/objection/cancellation pays nothing. The 72h HOT band floor is unchanged. |
| Appointment created (we asked) | 80 | 7d | Strong, but it is not their hand raised. |
| Home saved · saved search created | 60 | 7d | Declared intent — a pure browser tops out WARM; HOT requires a conversation or a hand-raise. |
| Inquiry · share · like | 55 | 7d | Preference facts with evidence attached. |
| **A browsing day** (views, searches, virtual tours) | 45 | 5d | ONE impulse per calendar day, whatever the volume — a binge is one day (absorbs v1's daily cap). |
| Email click | 40 (opens: 0) | 5d | Opens are corrupted — Apple MPP prefetches ~49% of them. |
| `is_backfill` rows | 0, always | — | Backfill never fires anything — D4, applied to scoring too. |

##### The five negative states (#100.1 — what the v1 sum was structurally blind to)

| State | Trigger | Effect |
|---|---|---|
| **PAUSED** | A stated pause classified from the lead's own words ("taking a break", "wait a bit") — live speech or the quoted speech inside an `ai_fact_captured` fact | Snaps to the nurture floor; behavior since lifts them to at most **50**; their quoted words render on the panel. Clears on a new hand-raise or a classified "we're back". |
| **FINANCING-BLOCKED** | Credit/lender evidence in notes | **Ceiling 45** (the one knob left tunable at signing) until a clearing note or the desk's 90-day review. |
| **TOUR CANCELLED** | The classified cancellation message | Revokes the tour impulse; never a positive beat. |
| **SUPPRESSED** | STOP / unsubscribe | The wall stands (band FROZEN) **and the score is 0**. |
| **CLOSED** | A terminal disposition (Trashed, Has Other Agent, …) | 0 until explicit reactivation (a re-disposition to Active). |

**The Bradley rule (conversation content is a signal).** Every inbound is classified before it
scores — cancellation · pause · positive · objection · neutral, by the declared vocabularies —
so "we're taking a break" finally moves the score **down** instead of paying +40 for arriving.
Uncertain classification fails toward neutral (toward scoring), and the same message never pays
both. Proof suite: `npm run test:scenario-reader` (the v2 blocks carry the Bradley fixture beat
for beat off his real record).
...
*The Addendum-3 simulator requirement it names now lives in its own canonical document —
**[THE AI TEST COCKPIT](/guides/ai-test-cockpit)** (Zach ruling #50, 2026-08-27).*
---
...
| the test cockpit's roleplay route | [THE AI TEST COCKPIT](/guides/ai-test-cockpit) |
...
### The real-person hold — live 2026-09-10 (item 075d1774)

**A draft aimed at any lead outside the test bench is always held for a human, whatever the autonomy
setting says.** `proposeCommunication` resolves the lead's audience through the same resolver the bench
grants use (`lib/ai/audience.ts` — the minted `is_test` column plus Zach's own enumerated row, never the
name-pattern predicate) and, for `world`, forces `pending_review` beside the directed hold and E5's
named-lead hold. A failure to resolve reads as `world`, so an unreadable lead is held, never released.
It carries no switch: it only ever adds a hold.

Why it exists: measured 2026-09-10, E5 welded "always held" to a named-lead **grant**, so a real person
with no grant and a `live` per-lead override (mig 0972) landed `authorized` — the only thing between
that row and a provider was the wire's `ai_master` rung, which the signed #257.174a sequence retires.

Proof: `npm run test:real-person-hold` (`lib/ai/real-person-hold.test.ts`) — a world fixture and a
bench fixture, both with `myli_autonomy='live'`, minted on `fixture.invalid` and deleted in `finally`.
Red-first at HEAD (the plant read `authorized`), then green: the bench control lands `authorized`, the
plant lands `pending_review`, the queue lists it by name untagged, a human Approve stamps the deciding
seat, and the wire refuses the approved plant **by a named rung** (today the test-lead-only ceiling,
`ai_test_mode`) with its ledger row.

**The turn-on is Zach's, and it is two switches he already owns:** `ai_test_mode.enabled → false`
(the ceiling over the wire, #257.188/.189a) and, while `ai_master` is off, a `named_lead` grant per
person behind `ai_named_lead_band` (mig 0881/0882). Neither is flipped by the build; both are OFF for
real people at HEAD.
...
|---|---|---|
| **§5 — this section** | **the LANGUAGE** — the prompt registry, prompting doctrine, per-lane prompt assembly, the working memory composer, the per-rung wording | where a prompt lives · how the voice/text/email prompt is built · what she is allowed to say |
...
## Shipped 2026-09-08 — the website chat, the back office, and what's still open

Live on prod (web deploy `1572683e00`, CRM deployed the same push). This section is written from
the night's lane receipts (`.claude/reports/grinder/webchat-blueprint/`,
`.claude/reports/grinder/myli-os/`, `.claude/reports/one-brain/`,
`.claude/reports/grinder/whole-loop/`, `.claude/reports/grinder/qa-bench/`) — read those for the
full measurement detail; this is what changed and why.

### The website chat now streams, acknowledges, and shows its work (rulings #257.322-.324)

Before this pass, prod was quietly serving the OLD chat modal against the NEW engine (a Railway
Docker build-arg for the client feature flags was never declared, so the flags always read
`undefined` in the browser bundle) — the screen sat on "Thinking…" for 5-7 seconds with no
indication anything was happening. Fixed by declaring the missing Dockerfile ARGs. With the real
modal live:

- **She streams.** First token in under a second; an acknowledgment line (her own words, one
  sentence, at most 15 words — never a scripted line) appears within about a second of a tap;
  named work-in-progress lines follow ("Checking what 10214 Conser Street is worth", "Comparing
  recent sales near 10214 Conser Street") drawn from the real tool call in progress, never
  composed; her answer then **streams as a draft** under those lines so the screen moves from the
  first token instead of sitting idle for 20-40 seconds.
- **The thread opens in the lead's own words.** A pill tap used to send only the pill's label as
  the chat bubble ("What can I afford?"); it now sends a first-person sentence ("Hey Myli, what
  can I afford?") as the actual message content, because the CRM files a lead's chat turns
  verbatim as their inbound message — the pill label was never something a person actually said.
- **Her replies render in paragraphs, bullets, and numbered lists**, not one 100-word wall of
  text. Two root causes, both fixed: the renderer only drew bold text before, and a repair pass
  that fixes double/compound questions was rejoining every sentence with a single space on every
  turn, flattening paragraph breaks even on replies that never needed repair.
- **One conversation thread per browser tab**, not a new one on every pill tap (see the home page
  doc's note on the same fix).
- **A refused turn no longer shows the scripted line "I lost the thread on that one for a
  second."** It ships a named reason (`floor: "guard-refused"`) and the screen draws a plain
  notice — "That one didn't come through." plus an **Ask again** button that resends the buyer's
  own last line — never invented copy standing in for her words.
- **Sign-in gained a phone-code entrance.** The sign-in sheet (desktop header and the mobile
  menu) offered only "Continue with Google" or an email code — a returning buyer who remembered
  only their mobile number had no door in. It now offers "Text me a code" on the same underlying
  phone-OTP road the chat already used, reusing the existing sign-in ticket flow (no new code
  road, no new table). "Get pre-approved" on the listing calculator now correctly signs a guest in
  with the same texted code once they submit their phone number (the code box had been silently
  dropped by the modal before this fix).

### Her numbers are her tools' numbers (the "numbers truth" pass)

Four classes of wrong or unsourced numbers, each traced to a tool that either didn't exist or
wasn't being trusted, and fixed at the tool:

- **Affordability now applies a cash law, not just an income rule.** The old calculator only
  asked what the buyer's INCOME could carry and treated a stated savings amount as "the down
  payment on whatever price falls out" — which could mean a 2.5% down payment on a loan no
  program writes, and no mortgage insurance in the payment. The new calculator holds two
  ceilings (what the income carries, what the cash — after closing costs — actually funds) and
  names which one is binding (`limiter: income | debts | cash`), with mortgage insurance folded
  into the payment above 80% loan-to-value. Example: $16,750/mo income, $0 debt, $20,000 saved
  used to price out at $630K-$815K; it now reads about **$365,000** (the cash ceiling), with the
  income-only ceiling ($746,623) and the savings needed to reach it named alongside.
- **A real commute tool.** She used to state a distance and drive time with no tool backing them
  (a number-truth guard let any number 60-or-under straight through). A new `commute` tool calls
  the same Google Routes API the geo pages already use, returns miles, minutes without traffic and
  minutes right now, and the guard now convicts any distance/time pairing that didn't come from a
  tool call.
- **Schools by name.** A market-analysis reader was filtering the listing's school fields for
  the wrong shape and always got back nothing, so she guessed a nearby school's name from
  general knowledge. A new `home_schools` tool reads the real per-listing assignment.
- **Days-on-market and tour-slot times now agree with the page.** Her sentence used to read a
  stale ingest-time snapshot while the card on screen computed live from the list date; both now
  read the same live figure. Tour slot times were computed on the server's clock (UTC on
  Railway) while the on-page calendar computes in the buyer's browser (Central) — the tool now
  builds its calendar on Central time explicitly.
- **Beds and the weekend are honored as constraints, not ignored.** A "3 bedroom" ask was being
  treated as a minimum and the first home shown could still be a 5-bedroom; results are now
  ranked at the named count first. "This weekend" now maps to the tool's day filter instead of
  being silently dropped in favor of the next three open days.
- **Her stated estimate now lands on the lead's CRM record.** The estimate tool used to file a
  number only when the website already had a signed-in `leadId` — so an anonymous buyer who
  answered all four money questions had their number computed and shown, but nothing was ever
  saved, and the CRM kept showing a stale pre-conversation figure after they registered. The
  estimate from the buyer's own committed answers (same cash-law calculator) now files at the
  moment their conversation adopts onto a lead.

### The CRM back office — what she writes down, and where two defects lived

- **A fabricated fact is now refused before it's written.** A cited row must actually hold the
  claim it's cited for — a chat message with no dollar figure in it can no longer become a fact
  reading "Under $500k preferred (stated)." The check compares MONEY amounts specifically (not
  bare numbers, so "three" vs. "3" doesn't misfire) and refuses by name when the cited text
  doesn't hold at least one of the amounts.
- **A recap question is no longer filed as a search.** The website fires a `ai_search` pixel on
  every chat turn, including plain replies like "yes, book it, whatever time works" — that
  pixel used to fall back to filing the raw sentence as a "recent search" fact. It's now filed
  only when the turn actually carries a search criterion, a saved search, or a real result count.
  Measured before the fix: 52% of all "Recent searches" facts ever written came from a typed
  question, not an actual search.
- **Chat cards now carry which run they came from** (`run_id`, promoted out of a JSON blob into
  its own column) so a card can be traced back to the conversation that produced it. Still open:
  the website-chat turns themselves (as opposed to playbook-driven turns) don't yet carry this,
  because the route logs the words before it resolves the run — named as a seam for whichever
  lane touches that ordering next.
- **Raw `**bold**` markdown was showing up literally in the CRM's chat card** (asterisks and
  all) — traced to the fact that there is no markdown renderer anywhere in the CRM (only two
  markdown-STRIPPING functions, used for outbound SMS/email hygiene). Not yet fixed — flagged for
  whichever lane builds the shared renderer this needs.

### Myli moves the stage

Zach's ruling #257.320: when a lender introduction or a tour booking completes inside the chat,
Myli should move the lead's pipeline stage to Appointment Set the same way a human would. Built:
`advanceStageOnMyliMoment()` resolves the lead's own pipeline (Buyer vs. Seller name their
"appointment set" stage differently) and its configured stage from the same setting
`advanceStageOnAppointment` already reads, then calls the one stage writer with Myli as the
actor, forward-only (a lead already further along never gets moved backward). One collision was
found and fixed in the same pass: a registration workflow's own opening step was resetting the
lead back to New Lead a couple of seconds after Myli moved it forward — workflow-driven stage
moves are now forward-only everywhere, measured against every `update_stage` step drawn in the
whole workflow estate (50 nodes, none of which ever meant to move a lead backward).

### Silence is now a card

When Myli deliberately holds off — she has a reason to wait rather than reach out — the feed now
files a quiet card naming the reason and the next scheduled touch, instead of leaving no trace at
all that a decision was made. New catalog kind `ai_touch_held` (data only, no new table). The
"watch" move kind on a playbook has existed in the schema since May but had zero production
writers before this pass; it's now wired through the playbook engine's fourth executor (send /
handoff / action / **watch** — one function per kind, never a branch inside another) and through
the scheduled-action road that releases a beat with no move to make.

**The grain: one card per reason per day, per run and move.** A caller that re-evaluates a standing
hold on a cadence would otherwise file a card per evaluation — one decision, restated into an
unreadable feed. So the same hold, restated the same day, collapses onto the card already there; a
**different** reason files its own card, because the reason is the whole point of the card and
collapsing two reasons into one would throw away the more informative half. A new day always files
again — "she is still holding off, a week later" is a fact a reader needs. On the released-beat road
the scheduled-action row is the key instead, so a retried release lands on the same card.

### The memory pipeline's coverage gauge (a real fix, not a wider net)

The hourly gauge that checks "did every kind of thing that happened to a lead in the last 7 days
get read by a memory pass" was reading 41% coverage with 18 kinds never seen. The kind list
wasn't the problem — every one of those kinds was already in scope. The problem was the LEAD
picker: a guard added days earlier (correctly, for a different bug) refused to pick any lead
whose TOTAL unprocessed debt exceeded 400 rows, even though the pass only ever reads the newest
400 rows per lead — so a lead that crossed 400 rows of debt once could never be picked again, and
by the time this was caught every lead in the estate with any debt at all had crossed that line.
The fix moves the guarantee to what the pass can actually read (is there an unprocessed row
inside the reader's own window) rather than the lead's total debt. Coverage moved from 41.4% to
45.6% after processing a single test lead; the estate-wide sweep clears the rest once the CRM
deploys, and 38 leads/~40,000 rows of debt older than any pass can reach are now counted by name
on every receipt rather than hidden behind a blanket refusal.

### Her own bookkeeping was eating her read window (the second half of the coverage fix)

The picker fix above moved the gauge from 41.4% to 64.6%, and there it stopped — 11 kinds still
reading never-seen, and the live picker returning **zero leads on every sweep** for a completely
different reason. Myli reads the newest 400 things on a lead. On the leads she knows best, almost
all 400 of them were her *own receipts*: every fact she captures files an `ai_fact_captured` card,
and at 5,030 of those in seven days it is the single highest-volume kind in the estate, against 833
property views. Measured on three real leads the day of the fix:

| lead | rows in the 400-row window that were her own receipts | what the window covered in time |
|---|---|---|
| Norma Herrera | 394 of 400 (99%) | 14 hours |
| Kristen Hansan | 385 of 400 (96%) | 12.7 hours |
| Morgan Activebuyer-TEST | 316 of 400 (79%) | 1.6 days |

That is a loop that tightens on itself: the more she learns about someone, the more receipts she
writes, the less time her window covers, and the more of what that person actually did falls out
the back of it before any pass can read it. It also meant her live conversation on those leads was
reading almost nothing but her own filing.

The catalog already said these kinds are not evidence — her workings, feed chrome (starring or
clearing a card), AI-gate stamps, cockpit bench events. Now they also stop **spending** the window:
the reader skips them, and the memory pass's floor skips them in exactly the same way, so the rule
"never stamp a row nobody read" holds unchanged. Nothing is hidden — the record block states what
it withheld in the same sentence it already uses for archived duplicates, and the facts those
receipts recorded still reach her from her memory, where they live. No kind list was widened, no
row is stamped unread, no migration.

Measured against the live database on the shipped queries: the picker goes from **0 leads to 25**
(2,500-odd rows readable on the first sweep), and the debt no pass can reach drops from 39,503 rows
to 34,829. On a real lead the effect is large — Norma Herrera's window goes from covering 14 hours
to covering **28 days**.

**The gauge does not go green on this, and that is worth saying plainly.** It moves from 11 kinds
never-seen to 8, and the eight survivors are all on a single test-band bench lead that files 1,351
non-receipt rows in six days — enough that 400 rows still only reaches back two days on that one
record. That is the paging problem, not this one, and a gauge held red by synthetic bench traffic is
its own question.

### Logged-out browsing stays on the lead, and reads as its own band

Ruling #257.320: a registered lead who logs out of houseloop.ai and keeps browsing should still
have that browsing attached to their record, visibly marked as happening while signed out. The
website's visitor cookie already survives logout (nothing ever cleared it) and already carried a
`signed_in` flag on every pixel event — the CRM's own writer was the gap: it only attached an
event to a lead when `signed_in === true`, so every logged-out view was written with no lead at
all. Fixed: the lead stays attached regardless of sign-in state, and a new `signed_out` flag on
the pixel-event tape (one column, read through the existing `source_id` lookup — no new join, no
copy of the fact) drives a blue **"Logged out history"** band in the activity feed, the same
chrome as the existing "Before they registered" and Follow Up Boss legacy bands. It **groups
each contiguous logged-out stretch where it happened**, rather than hoisting all logged-out
activity to one place — a person can sign out, browse, and sign back in more than once, and the
feed's chronological order is never disturbed to make that fit a single block.

### The Playbook Shelf rebuild — still RED on the QA judge

The condemned old playbooks settings page was replaced with a real gallery (21 cards) + per-
playbook canvas (small prompt, example bank, tool-grant cards, an Academy certification chip,
limits). Several defects from the first judge pass were fixed — a status pill that read the
opposite of what the engine actually does, an editor that pointed at a second, never-read prompt
slot for six of the canvases, a "0 tools" reading that actually meant "every tool," a "no edits
yet" chip shown over 885 characters of real prompt text. **The second judge pass still failed**
on: a strapline claiming certain tools are "granted, never listed" sitting two inches from cards
that say the opposite about the same six tools; three sentences that don't match their own data
(a prompt-edit history chip claiming an edit that isn't in the history; "she is refused the
moment she reaches for it" over 13 tools that in fact work); and copy-desk-level issues (a
harvester's changelog leaking into user-facing text, code fences rendering literally on screen).
Not ready to call done — the next round is a copy and strapline-accuracy pass, not a rebuild.

### What is built but not yet on prod, as of this writing

A same-night follow-up round (walk-3 fixes) landed several more defects — a "Get pre-approved"
button on mobile covered by the page's own sticky bar (a stacking-context bug, fixed with a
portal), a React hydration error on every listing load (a tour-time widget computing from
`new Date()` on the server instead of the visitor's own clock), and a real cut to turn latency
(a redundant "repackage the words you already wrote" model round, worth 16-19 seconds on turns
that end in plain prose) — but as of this section being written, those commits are **built and
tested, not yet pushed**. Check `git log origin/houseloop-app` or `houseloop.ai/api/health` before
treating any of that as live.
...
| `command-center/myli-playbooks-spec` | §2 — governance, the three launch playbooks, **the signature block + three addenda**. Its Addendum-3 simulator spec went to [THE AI TEST COCKPIT](/guides/ai-test-cockpit) |
| `guides/ai-programming` | §3 — the Director's build status and the Observer; §6 — the whole language layer |
| `legacy/guides/test-harness` | folded OUT to [THE AI TEST COCKPIT](/guides/ai-test-cockpit) per ruling #50 — what the shipped cockpit actually is |
